...
def calculate_lambda(
    ...
    if model_type in ("simple", "ambient"):
        ...
        msg = f"model_type was set to {model_type}, which is not implemented."
...
def calculate_mu(
    ...
    elif model_type in ("swapping", "full"):
...
class RemoveBackgroundPyroModel(nn.Module):
    ...
    def __init__(
        ...
        if self.model_type in ("full", "swapping"):
        ...
        if use_cuda:
            ...
            self.cuda()
            try:
                for _key, value in self.encoder.items():
                    value.cuda()
            except KeyError:
                pass
            self.device = "cuda"
    ...
    def guide(self, x: torch.Tensor):
        ...
        with pyro.plate("data", x.shape[0], use_cuda=self.use_cuda, device=self.device):
            # Sample swapping fraction rho.
            if self.include_rho:
                pyro.sample("rho", dist.Beta(rho_alpha, rho_beta).expand_by([x.size(0)]))
            ...
            if self.include_empties:
                ...
                pyro.sample("d_cell", dist.LogNormal(loc=d_cell_loc_gated, scale=d_cell_scale))
...
def to_ndarray(x: Union[Number, np.ndarray, torch.Tensor]) -> np.ndarray:
    ...
    if type(x) is np.ndarray:
        ...
        msg = f"to_ndarray() received input of type {type(x)}"
        raise TypeError(msg)
