...
class DataLoader:
    ...
    def __init__(
        ...
        self.cell_batch_size = int(batch_size * (1.0 - fraction_empties))
    ...
    def set_state(self, ind_list: np.ndarray, ptr: int):
        ...
        assert self.ptr <= len(self.ind_list), (
            f"Problem setting dataloader state: pointer ({ptr}) is outside the length of the ind_list ({len(ind_list)})"
        )
    ...
    def __next__(self):
        ...
        if remaining_cells < consts.SMALLEST_ALLOWED_BATCH:
            ...
            n_empties = int(cell_inds.size * (self.fraction_empties / (1 - self.fraction_empties)))
            if self.empty_ind_list.size > 0:
                # This does not happen for 'simple' model.
                empty_inds = np.random.choice(self.empty_ind_list, size=n_empties, replace=True)

                if empty_inds.size > 0:
                    csr_list = [self.dataset[cell_inds, :], self.empty_drop_dataset[empty_inds, :]]
                else:
...
def prep_sparse_data_for_training(
    ...
    training_indices = [idx for idx in range(dataset.shape[0]) if training_mask[idx]]
    test_indices = [idx for idx in range(dataset.shape[0]) if not training_mask[idx]]
    ...
    training_indices_empty = [idx for idx in range(empty_drop_dataset.shape[0]) if training_mask_empty[idx]]
    test_indices_empty = [idx for idx in range(empty_drop_dataset.shape[0]) if not training_mask_empty[idx]]
