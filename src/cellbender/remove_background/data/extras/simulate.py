...
def generate_sample_inferred_model_dataset(
    ...
    random_seed: int = 0,
) -> dict[str, Union[float, np.ndarray, sp.csr_matrix]]:
    ...
    for i, data in enumerate(data_loader):
        enc = model.encoder(x=data, chi_ambient=chi_ambient, cell_prior_log=model.d_cell_loc_prior)
    ...
    if n_clusters_ < len(cells_of_each_type):
        print(
            f"WARNING: DBSCAN found only {n_clusters_} clusters, but the input "
    ...
    if plot_pca:
        ...
        for label in np.unique(labels):
            if label == -1:
                ...
                color = None
            plt.plot(pca[labels == label, 0], pca[labels == label, 1], ".", color=color, label=label)
    ...
    z_cluster = []
    for label, n_cells in zip(np.unique(labels)[: len(cells_of_each_type)], cells_of_each_type, strict=False):
        if n_cells == 0:
            continue
        logic = labels == label
    ...
    i = 0
    for t, (num_cells, z_chunk) in enumerate(zip(cells_of_each_type, z_cluster, strict=False)):
...
def generate_sample_dirichlet_dataset(
    ...
    if vector_to_add_to_chi_ambient is not None:
        assert len(vector_to_add_to_chi_ambient) == n_genes, (
            f"vector_to_add_to_chi_ambient {vector_to_add_to_chi_ambient.shape} must be of length n_genes ({n_genes})"
        )
    ...
    for umi in cell_mean_umi:
        assert umi > empty_mean_umi, f"cell_size_mean_umi ({umi}) must be > empty_mean_umi ({empty_mean_umi})"
    ...
    elif vector_to_add_to_chi_ambient is not None:
        print(
            "You specified both `chi_ambient` and `vector_to_add_to_chi_ambient`. "
    ...
    # Sample counts from cell-containing droplets.
    i = 0
    for t, (num_cells, celltype_umi) in enumerate(zip(cells_of_each_type, cell_mean_umi, strict=False)):
...
def draw_random_chi(alpha: float = 1.0, n_genes: int = 10000) -> torch.Tensor:
...
def sample_from_inferred_model(
    num: int, model: "RemoveBackgroundPyroModel", z: Optional[torch.Tensor] = None, y: Optional[torch.Tensor] = None
) -> dict[str, np.ndarray]:
    ...
    params = {
        name: (
            model_trace.nodes[name]["value"].unconstrained()
        ...
        if ("$$$" not in name and name not in do_list)
    }  # the do-samples are dummies
    ...
    c_real = pyro.distributions.NegativeBinomial(total_count=model_output["alpha"], logits=logit).to_event(1).sample()
    ...
    return {
        "counts_real": c_real.detach().cpu().numpy(),
...
def sample_from_model(
    ...
    assert (1.0 - chi_ambient.sum()).abs() < 1e-6, f"chi_ambient must sum to 1, but it sums to {chi_ambient.sum()}"
    assert len(chi_ambient.shape) == 1, "chi_ambient should be 1-dimensional."
    assert chi.shape[-1] == chi_ambient.shape[-1], (
        "chi and chi_ambient must be the same size in the rightmost dimension."
    )
    assert (1.0 - chi.sum()).abs() < 1e-6, f"chi must sum to 1, but it sums to {chi.sum()}"
    ...
    c_real = torch.distributions.NegativeBinomial(total_count=alpha, logits=logits).sample()
...
def sample_from_dirichlet_model(
    num: int,
    alpha: np.ndarray,
    d_mu: float,
    d_sigma: float,
    ...
    assert np.abs(1.0 - chi_ambient.sum()) < 1e-10, f"chi_ambient must sum to 1, but it sums to {chi_ambient.sum()}"
    assert len(chi_ambient.shape) == 1, "chi_ambient should be 1-dimensional."
    assert alpha.shape[0] == chi_ambient.size, "alpha and chi_ambient must be the same size in the rightmost dimension."
    assert num > 0, f"num must be > 0, but was {num}"
    assert eps_param > 1, f"eps_param must be > 1, but was {eps_param}"
    ...
    # Draw epsilon ~ Gamma(eps_param, 1 / eps_param)
    epsilon = rng.gamma(shape=eps_param, scale=1.0 / eps_param, size=num)
...
def get_dataset_dict_as_anndata(
    ...
    if "counts_true" in d.keys() and "counts_bkg" in d.keys():
        adata = anndata.AnnData(
            X=(d["counts_true"] + d["counts_bkg"]).astype(np.float32),
        ...
        adata = anndata.AnnData(
            X=d.pop("matrix").astype(np.float32), obs={"barcode": d.pop("barcodes")}, var={"gene": d.pop("gene_names")}
        )
...
def write_simulated_data_to_h5(
    output_file: str, d: dict[str, Union[float, np.ndarray, sp.csr_matrix]], cellranger_version: int = 3
) -> bool:
    ...
    return write_matrix_to_cellranger_h5(
        ...
        genomes=(np.array(["simulated"] * d["gene_names"].size) if ("genome" not in d.keys()) else d["genome"]),
        barcodes=d["barcodes"],
...
def write_simulated_truth_to_h5(
    output_file: str, d: dict[str, Union[float, np.ndarray, sp.csr_matrix]], cellranger_version: int = 3
) -> bool:
    """Helper function to write the full (noisy) simulate dataset to an H5 file.
    ...
    global_latents = {f"truth_{key.replace('chi_ambient', 'ambient_expression')}": d[key] for key in extra_latents}
    ...
    return write_matrix_to_cellranger_h5(
        ...
        cellranger_version=cellranger_version,
        local_latents={
            "truth_cell_label": d["droplet_labels"],
