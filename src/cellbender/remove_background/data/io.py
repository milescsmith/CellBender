...
def write_matrix_to_cellranger_h5(
    ...
    metadata: dict[str, Optional[Union[np.ndarray, int, str, dict]]] = {},
) -> bool:
...
def write_posterior_coo_to_h5(
    ...
    regularized_posterior_kwargs: Optional[dict] = None,
) -> bool:
    ...
    with tables.open_file(
        output_file, "w", title="CellBender remove-background posterior noise count probabilities"
    ) as f:
        # metadata
        ...
        if noise_count_offsets != {}:
            f.create_carray(extras, "noise_count_offsets_keys", obj=list(noise_count_offsets.keys()), filters=filters)
            f.create_carray(
                extras, "noise_count_offsets_values", obj=list(noise_count_offsets.values()), filters=filters
            )
        ...
        f.create_carray(
            group, "shape", atom=tables.Int64Atom(), obj=np.array(posterior_coo.shape, dtype=np.int64), filters=filters
        )

        # regularized posterior COO
        if regularized_posterior_coo is not None:
            group = f.create_group(
                "/", "regularized_posterior_noise_log_prob", "Regularized posterior noise count log probabilities"
            )
            ...
            f.create_carray(
                ...
                obj=np.array(regularized_posterior_coo.shape, dtype=np.int64),
                filters=filters,
            )
        ...
        if posterior_kwargs is not None:
            ...
            reg_kwargs_group = f.create_group("/", "kwargs_regularized", "Function arguments for regularized posterior")
...
def load_posterior_from_h5(filename: str) -> dict[str, Union[sp.coo_matrix, np.ndarray]]:
    ...
    with tables.open_file(filename, "r") as f:
        # read metadata
        barcode_inds = f.root.metadata.barcode_inds.read()
        feature_inds = f.root.metadata.barcode_inds.read()
        if hasattr(f.root.metadata, "noise_count_offsets_keys"):
            noise_count_offsets_keys = f.root.metadata.noise_count_offsets_keys.read()
            noise_count_offsets_values = f.root.metadata.noise_count_offsets_values.read()
            noise_count_offsets = dict(zip(noise_count_offsets_keys, noise_count_offsets_values, strict=False))
        else:
            noise_count_offsets = {}

        def _read_coo(group: tables.Group) -> sp.coo_matrix:
            data = group.log_prob.read()
            row = group.m_index.read()
            col = group.noise_count.read()
    ...
    if posterior_coo.row.size != barcode_inds.size:
        logger.warning(
            f"Number of barcode_inds ({barcode_inds.size}) "
    ...
    if posterior_coo.row.size != feature_inds.size:
        logger.warning(
            f"Number of feature_inds ({feature_inds.size}) "
            f"in {filename} does not match the number expected from "
            f"the sparse COO matrix ({posterior_coo.shape[0]})."
        )
...
def get_matrix_from_cellranger_h5(filename: str) -> dict[str, Union[sp.csr_matrix, np.ndarray]]:
    ...
    with tables.open_file(filename, "r") as f:
        ...
        if cellranger_version == 2:
            ...
            for group in f.walk_groups():
                try:
                    ...
                    fnames_this_genome = group.gene_names.read()
                    feature_names.extend(fnames_this_genome)
                    feature_ids.extend(group.genes.read())
        ...
        # There is only the 'matrix' group.
            ...
            # Read in data for this genome, and put it into a
            # scipy.sparse.csc.csc_matrix
            barcodes = f.root.matrix.barcodes.read()
            data = f.root.matrix.data.read()
            indices = f.root.matrix.indices.read()
            indptr = f.root.matrix.indptr.read()
            shape = f.root.matrix.shape.read()
            csc_list.append(sp.csc_matrix((data, indices, indptr), shape=shape))

            # Read in 'feature' information
            feature_group = f.get_node(f.root.matrix, "features")
            feature_names = feature_group.name.read()

            try:
                feature_types = feature_group.feature_type.read()
            except tables.NoSuchNodeError:
                # This exists in case someone produced a file without feature_type.
                pass
            try:
                feature_ids = feature_group.id.read()
            except tables.NoSuchNodeError:
                # This exists in case someone produced a file without feature id.
                pass
            try:
                genomes = feature_group.genome.read()
            except tables.NoSuchNodeError:
...
def get_matrix_from_bd_rhapsody(filename: str) -> dict[str, Union[sp.csr_matrix, np.ndarray]]:
    ...
    count_matrix = sp.csc_matrix((data, (row, col)), shape=(len(barcodes), len(gene_names)), dtype=np.float)
