...
class SingleCellRNACountsDataset:
    ...
    def __init__(
        ...
        if expected_cell_count is not None:
            ...
            self.priors.update(
                get_cell_count_given_expected_cells(umi_counts=counts, expected_cells=expected_cell_count)
            )
            if (expected_cell_count + consts.NUM_EMPTIES_INCREMENT) > self.priors["total_droplets"]:
                ...
                if total_droplet_barcodes is None:
                    ...
                    self.priors.update(
                        get_empty_count_given_expected_cells_and_total_droplets(
                            umi_counts=counts, expected_cells=self.priors["expected_cells"], total_droplets=total_drops
                        )
                    )
        if total_droplet_barcodes is not None:
            ...
            self.priors.update(
                get_empty_count_given_expected_cells_and_total_droplets(
                    umi_counts=counts,
                    expected_cells=self.priors["expected_cells"],
    ...
    def _exclude_feature_types(self):
        ...
        if self.data["feature_types"] is None:
            if len(self.exclude_features) > 0:
                logger.warning(
                    f"WARNING: specified --exclude-feature-types "
                ...
                logger.info(
                    "No feature type information specified in the input file. Including all features in the analysis."
                )
            return
        ...
        for feature in self.exclude_features:
            ...
            logic = np.array([f not in self.exclude_features for f in feature_type_array], dtype=bool)
            inclusion_logic = np.logical_and(inclusion_logic, logic)
            logger.info(f"    - This results in the exclusion of {len(feature_type_array) - logic.sum()} features.")
    ...
    def _clean_features(self, gene_blacklist: list[int] = []):
        ...
        if len(gene_blacklist) > 0:
            ...
            inclusion_logic = np.array([g not in gene_blacklist for g in range(len(self.analyzed_gene_logic))])
            logger.info(f"Ignoring {np.logical_not(inclusion_logic).sum()} specified features.")
            self.analyzed_gene_logic = np.logical_and(self.analyzed_gene_logic, inclusion_logic)

        if self.analyzed_gene_logic.sum() == 0:
            logger.warning(
                "No features remain after eliminating zero-count and ignored features. Terminating analysis."
            )
            raise AssertionError(
                "No features with nonzero counts remain. Check "
    ...
    def _trim_noiseless_features(self):
        ...
        ambient_counts_in_cells_g = max(self.priors["expected_cells"], 5000) * mean_counts_per_empty_g

        # Include features that are projected to have noise above the low limit.
        inclusion_logic_subset = ambient_counts_in_cells_g > self.ambient_counts_in_cells_low_limit
        ...
        self.analyzed_gene_logic = np.logical_and(self.analyzed_gene_logic, inclusion_logic)
        logger.info(
            f"Excluding {np.logical_not(inclusion_logic).sum()} features that "
    ...
    def _trim_droplets(self):
        ...
        if not self.include_empties:
            ...
            logger.info(
                f"Excluding empty droplets due to '{self.model_name}' model: "
        ...
        # If not using the simple model, include empty droplets.
            ...
            assert num_barcodes_above_umi_cutoff > 0, (
                f"There are no barcodes with UMI counts over the lower cutoff of {low_count_cutoff}"
            )
            ...
            empty_droplet_sorted_barcode_inds = np.arange(
                total_droplet_barcodes, num_barcodes_above_umi_cutoff, dtype=int
            )  # The entire range
            ...
            cell_prob = (n_cells / total_droplet_barcodes) * (1.0 - self.fraction_empties)
            self.priors["cell_logit"] = np.log(cell_prob) - np.log(1.0 - cell_prob)

            logger.info(
                f"Using {n_cells} probable cell "
                f"barcodes, plus an additional "
                f"{total_droplet_barcodes - n_cells} barcodes, "
                f"and {len(self.empty_barcode_inds)} empty "
                f"droplets."
            )
            logger.info(f"Largest surely-empty droplet has {int(self.empty_UMI_threshold)} UMI counts.")

            if (low_count_cutoff == self.low_count_threshold) and (len(self.empty_barcode_inds) == 0):
                logger.warning(
                    "Warning: few empty droplets identified. Low "
    ...
    def get_count_matrix(self) -> sp.csr_matrix:
        ...
        trimmed_bc_matrix = self.data["matrix"][self.analyzed_barcode_inds, :].tocsc()
    ...
    def get_count_matrix_empties(self) -> sp.csr_matrix:
        ...
        # Return the count matrix for selected barcodes and genes.
        trimmed_bc_matrix = self.data["matrix"][self.empty_barcode_inds, :].tocsc()
        trimmed_matrix = trimmed_bc_matrix[:, self.analyzed_gene_inds].tocsr()
        return trimmed_matrix
    ...
    def restore_eliminated_features_in_cells(
        self, inferred_count_matrix: sp.csc_matrix, cell_probabilities_analyzed_bcs: np.ndarray
    ) -> sp.csc_matrix:
        ...
        out = overwrite_matrix_with_columns_from_another(
            mat1=inferred_count_matrix, mat2=self.data["matrix"], column_inds=self.analyzed_gene_inds
        )
