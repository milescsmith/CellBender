...
def test_save_and_load_pyro_checkpoint(tmpdir_factory, batch_size_n):
    ...
    _check_all_close(_get_params(model_ckpt.encoder), _get_params(initial_model.encoder))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (optim.ExponentialLR, {"optimizer": torch.optim.SGD, "optim_args": {"lr": 0.01}, "gamma": 0.9}),
    ],
)
def test_optimizer_checkpoint_restart(Optim, config, tmpdir_factory):
    ...
    def get_snapshot(optimizer):
        ...
        if type(optimizer) == optim.lr_scheduler.PyroLRScheduler:
            lr = next(iter(optimizer.optim_objs.values())).get_last_lr()[0]
        else:
            lr = next(iter(optimizer.optim_objs.values())).param_groups[0]["lr"]
        s.update({"lr": f"{lr:.4f}"})
        return s
    ...
    for i, (_ac, _ex) in enumerate(zip(actual, expected, strict=False)):
        if i == 4:
            pass

    # ensure actual matches expected
    for actual_t, expected_t in zip(actual, expected, strict=False):
        actual_t.pop("lr")
        expected_t.pop("lr")
        for actual_value, expected_value in zip(actual_t.values(), expected_t.values(), strict=False):
