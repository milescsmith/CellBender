...
def test_dense_to_sparse_op_torch(simulated_dataset, cuda):
    ...
    data_loader = DataLoader(
        d["matrix"], empty_drop_dataset=None, batch_size=5, fraction_empties=0.0, shuffle=False, use_cuda=cuda
    )
    ...
    out = sp.csc_matrix((counts, (barcodes, genes)), shape=d["matrix"].shape)
...
def test_log_prob_sparse_to_dense():
    ...
    mat_8 = todense_fill(coo=coo, fill_value=8.0)

    def truth(x):
        return np.array([[0.0, -1.0], [x, x], [-2.0, 0.0], [-4.0, -2.0], [0.0, x]])

    np.testing.assert_array_equal(mat, truth(-np.inf))
    np.testing.assert_array_equal(mat_nan, truth(np.nan))
    np.testing.assert_array_equal(mat_8, truth(8.0))


@pytest.mark.parametrize(
    "mat1, mat2, col_inds",
    [
        (sp.csc_matrix([[1, 2], [3, 4]]), sp.csc_matrix([[0, 0], [0, 0]]), [0]),
        (sp.csc_matrix([[1, 2], [3, 4], [5, 6]]), sp.csc_matrix([[0, 0], [0, 0], [0, 0]]), [1]),
        (
            sp.csc_matrix(np.random.poisson(lam=2.0, size=(10, 10))),
            sp.csc_matrix(np.random.poisson(lam=2.0, size=(10, 10))),
            [0, 1, 2, 3],
        ),
    ],
)
def test_overwrite_matrix_with_columns_from_another(mat1: sp.csc_matrix, mat2: sp.csc_matrix, col_inds: np.ndarray):
...
@pytest.mark.parametrize(
    "mat, row_inds",
    [
        (sp.csc_matrix([[1, 2], [3, 4]]), [0]),
        (sp.csc_matrix([[1, 2], [3, 4], [5, 6]]), [1]),
        (sp.csc_matrix(np.random.poisson(lam=2.0, size=(10, 10))), [0, 1, 2, 3]),
    ],
)
