...
from cellbender.remove_background.train import evaluate_epoch, train_epoch
...
def test_one_cycle_scheduler(dropped_minibatch, cuda):
    ...
    # Set up dataloaders.
    train_loader, _ = prep_data_for_training(
    ...
    svi = SVI(model=_model, guide=_guide, optim=scheduler, loss=pyro.infer.trace_elbo.Trace_ELBO())
    ...
    torch.testing.assert_close(lr[-1], final_lr, rtol=1, atol=1e-7), "Final learning rate for scheduler seems off"
    torch.testing.assert_close(max(lr), max_lr, rtol=1, atol=1e-7), "Max learning rate in scheduler seems off"
