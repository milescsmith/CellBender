...
from cellbender.remove_background.sparse_utils import dense_to_sparse_op_torch, log_prob_sparse_to_dense, todense_fill
from cellbender.remove_background.tests.conftest import simulated_dataset, sparse_matrix_equal, tensors_equal
...
def log_prob_coo_base() -> dict[str, Union[sp.coo_matrix, np.ndarray, dict[int, int]]]:
    ...
    shape = list(m.shape)
    shape[0] = shape[0] + 1
    offset_dict = dict(zip(range(1, len(m) + 1), [0] * len(m) + [1], strict=False))  # noise count offsets (last is 1)
...
def test_PRmu(log_prob_coo, fpr, per_gene, n_chunks, cuda):
    ...
    torch.tensor(log_prob_sparse_to_dense(log_prob_coo["coo"])).float()

    estimator = Mean(index_converter=index_converter)
    mean_noise_csr = estimator.estimate_noise(
        noise_log_prob_coo=log_prob_coo["coo"], noise_offsets=log_prob_coo["offsets"], device="cuda" if cuda else "cpu"
    )
    ...
    if fpr == 0.0:
        torch.testing.assert_close(
            actual=map_noise.sum().float(), expected=torch.tensor(mean_noise_csr.sum()).float(), rtol=1, atol=1
        )
    elif fpr == 1.0:
        torch.testing.assert_close(
            actual=map_noise.sum().float(), expected=torch.tensor(count_matrix.sum()).float(), rtol=1, atol=1
        )
    else:
...
def test_save_and_load(tmpdir_factory, blank_noise_offsets, m):
    ...
    for attr in [
        ...
        elif (type(val1) == dict) and (val1 != {}) and (type(next(iter(val1.values()))) == np.ndarray):
            for k in val1.keys():
