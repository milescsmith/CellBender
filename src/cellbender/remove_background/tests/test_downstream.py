...
from cellbender.remove_background.tests.conftest import SavedFileH5, h5_file, h5_v3_file_post_inference
...
def test_anndata_from_inferred_h5(h5_v3_file_post_inference: SavedFileH5, analyzed_bcs_only):
    ...
    for key in relevant_keys:
        assert key in extra_columns, f'Saved .uns information "{key}" is missing from adata: {adata.uns.keys()}'


def test_load_anndata_from_input_and_output(h5_file, h5_v3_file_post_inference):
    adata = load_anndata_from_input_and_output(
        input_file=h5_file.name,
        output_file=h5_v3_file_post_inference.name,
        gene_expression_encoding_key="gene_expression_encoding",
        analyzed_barcodes_only=False,
    )
    assert h5_file.shape == adata.shape, "Shape of loaded adata is not correct when loading all barcodes"
    for key in (
        h5_v3_file_post_inference.local_keys
