...
def test_simulate_save_load_h5(simulated_dataset, filetype, h5_v2_file, h5_v2_file_missing_ids, h5_v3_file):
    ...
    assert_loaded_matches_saved(
        d=simulated_dataset, loaded=loaded, keys=saved_h5.keys, cellranger_version=saved_h5.version
    )
...
def save_mtx(tmpdir_factory, simulated_dataset, version: int) -> str:
    ...
    if version == 3:
        ...
        msg = f"Test problem: version is {version}, but [2, 3] allowed"
...
def save_dge(tmpdir_factory, simulated_dataset, do_gzip) -> str:
    ...
    with load_fcn(filename, "wb") as f:
        ...
        for g, vals in zip(simulated_dataset["gene_names"], row_generator(simulated_dataset["matrix"]), strict=False):
