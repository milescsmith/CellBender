...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a Terra-compatible data table TSV from cromshell benchmarking outputs."
    )
    parser.add_argument(
        "-w",
        "--workflows",
    ...
    parser.add_argument(
        "-n", "--note", type=str, required=True, dest="note", help="Annotation describing the git commit"
    )
    parser.add_argument("-o", "--output", type=str, required=True, dest="output_file", help="Output TSV file")
...
def find_cromshell_workflow_ids_dates(grep: str) -> tuple[list[str], list[str]]:
    ...
    out = subprocess.run(["cromshell", "list"], capture_output=True, check=False)
    s = out.stdout.decode()
...
def metadata_from_workflow_id(workflow: str) -> tuple[str, str, Optional[str]]:
    ...
    # git hash
    output = grep_from_command(["cromshell", "metadata", workflow], grep='"git_hash":')
    git_hash = output.decode().split('"git_hash": ')[-1].lstrip('"').split('"')[0]

    # input file
    output = grep_from_command(
        ["cromshell", "metadata", workflow], grep="run_cellbender_benchmark.cb.input_file_unfiltered"
    )
    input_file = "gs://" + output.decode().split("gs://")[-1].split('"')[0]

    # truth file
    output = grep_from_command(["cromshell", "metadata", workflow], grep="run_cellbender_benchmark.cb.truth_file")
...
if __name__ == "__main__":
    ...
    if len(list_of_tuples) > 0:
        git_hashes, input_h5s, truth_h5s = zip(*list_of_tuples, strict=False)

        run_ids = [sample + "_" + git for sample, git in zip(samples, git_hashes, strict=False)]

        df = pd.DataFrame(
            data={
                "entity:sample_id": run_ids,
