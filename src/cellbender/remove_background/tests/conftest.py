...
def sparse_matrix_equal(mat1, mat2):
    ...
    if type(mat1) == sp.coo_matrix:
        ...
        msg = f"Error with test functions: sparse_matrix_equal() was called with a {type(mat1)}"
        raise ValueError(msg)


def csc_or_csr_equal(mat1: sp.csc_matrix, mat2: sp.csc_matrix):
    """Fast assertion that CSC sparse matrices are equal"""
    return (mat1 != mat2).sum() == 0


def coo_equal(mat1: sp.csc_matrix, mat2: sp.csc_matrix):
...
def tensors_equal(a: torch.Tensor, b: torch.Tensor):
...
class SavedFileH5:
    ...
    def __repr__(self):
        return f"Saved h5 file ({self.shape}) at {self.name} (CellRanger v{self.version} with keys {self.keys})"
...
def simulated_dataset():
    ...
    d["feature_types"] = np.array(
        ["Gene Expression"] * (d["gene_names"].size - 50)
        + ["Antibody Capture"] * 40
...
def h5_v2_file(tmpdir_factory, simulated_dataset) -> "SavedFileH5":
    ...
    yield SavedFileH5(
        name=filename, keys=["gene_names", "barcodes", "gene_ids"], version=cellranger_version, shape=d["matrix"].shape
    )
    shutil.rmtree(str(tmp_dir))
...
def h5_v2_file_missing_ids(tmpdir_factory, simulated_dataset) -> "SavedFileH5":
    ...
    yield SavedFileH5(
        name=filename, keys=["gene_names", "barcodes"], version=cellranger_version, shape=d["matrix"].shape
    )
    shutil.rmtree(str(tmp_dir))
...
def h5_file(request, h5_v2_file, h5_v3_file):
    if request.param == 2:
        ...
        msg = f"Test error: requested v{request.param} h5 file"
        raise ValueError(msg)
