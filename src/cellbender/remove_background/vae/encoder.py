...
class EncodeNonZLatents(nn.Module):
    ...
    def forward(
        ...
        p_extra_features = torch.cat(
            (log_sum, log_nnz, overlap, torch.linalg.vector_norm(z.detach(), ord=2, dim=-1, keepdim=True)), dim=-1
        )
        eps_extra_features = torch.cat(
            (log_sum, log_nnz, eps_overlap, torch.linalg.vector_norm(z.detach(), ord=2, dim=-1, keepdim=True)), dim=-1
        )
