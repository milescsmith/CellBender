...
class EstimationMethod(ABC):
    ...
    def _estimation_array_to_csr(
        ...
        return _estimation_array_to_csr(
            index_converter=self.index_converter, data=data, m=m, noise_offsets=noise_offsets, dtype=dtype
        )
...
def _mckp_chunk_estimate_noise(
    ...
    map_csr = _estimation_array_to_csr(
        data=map_dict["result"], m=map_dict["m"], noise_offsets=noise_offsets, index_converter=index_converter
    )
    ...
    lookup_map_from_m = dict(zip(map_dict["m"], map_dict["result"], strict=False))
    ...
    step_direction_lookup_from_m = dict(zip(df["m"], df["step_direction"], strict=False))
    ...
    steps_csr = _estimation_array_to_csr(
        data=vc_df["counts"], m=vc_df["m"], noise_offsets=None, index_converter=index_converter
    )
    ...
    def estimate_noise(
        ...
        **kwargs,
        ...
        if use_multiple_processes:
            ...
            chunk_logic_list = self._gene_chunk_iterator(noise_log_prob_coo=noise_log_prob_coo, n_chunks=n_chunks)
            ...
            for i, logic in enumerate(
                self._gene_chunk_iterator(noise_log_prob_coo=noise_log_prob_coo, n_chunks=n_chunks)
            ):
...
def chunked_iterator(
    ...
    for row_m_values in row_m_value_chunks:
        ...
        chunk_coo = sp.coo_matrix(
            (coo.data[logic], (rows, cols)), shape=(len(unique_row_values), len(unique_col_values))
        )
...
def _parallel_pandas_apply(
    ...
    """
    groupby_val, group_df_list = zip(*[(group_val, group_df) for group_val, group_df in df_grouped], strict=False)
