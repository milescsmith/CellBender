...
class TorchNegativeBinomialPoissonConv(Distribution):
    ...
    arg_constraints = {"mu": constraints.positive, "alpha": constraints.positive, "lam": constraints.positive}
    ...
    def __init__(self, mu, alpha, lam, max_poisson, validate_args=None):
        ...
        if isinstance(mu, Number) and isinstance(alpha, Number) and isinstance(lam, Number):
    ...
    def expand(self, batch_shape, _instance=None):
        new = self._get_checked_instance(TorchNegativeBinomialPoissonConv, _instance)
        ...
        super(TorchNegativeBinomialPoissonConv, new).__init__(batch_shape, validate_args=False)
    ...
    def _gamma_log_prob(alpha, beta, value):
        return alpha * beta.log() + (alpha - 1) * value.log() - beta * value - alpha.lgamma()
    ...
    def log_prob(self, value):
        ...
        neg_binom_log_prob_zero = self._neg_binom_log_prob_zero(mu[zero_indices], alpha[zero_indices])
        ...
        neg_binom_log_prob_zero = self._neg_binom_log_prob_zero(mu[one_indices], alpha[one_indices])
        poisson_log_prob_one = self._poisson_log_prob_one(lam[one_indices])
        neg_binom_log_prob_one = self._neg_binom_log_prob_one(mu[one_indices], alpha[one_indices])
        total_log_prob_one = torch.logsumexp(
            torch.cat(
                (
                    (poisson_log_prob_zero + neg_binom_log_prob_one).unsqueeze(-1),
                    (poisson_log_prob_one + neg_binom_log_prob_zero).unsqueeze(-1),
                ),
                dim=-1,
            ),
            dim=-1,
        ...
        poisson_values_low = torch.clamp(torch.min(poisson_values_low, (summation_obs - self.max_poisson).int()), min=0)

        increment_values = torch.arange(0, self.max_poisson + 1, dtype=value.dtype, device=value.device).expand(
            (*summation_obs.shape, -1)
        )
        ...
        poisson_log_prob_rest = self._poisson_log_prob(lam[summation_indices].unsqueeze(-1), poisson_values.float())
        neg_binom_log_prob_rest = self._neg_binom_log_prob(
            mu[summation_indices].unsqueeze(-1), alpha[summation_indices].unsqueeze(-1), nb_values.float()
        )
        ...
        total_log_prob_rest = torch.where(inf_mask, self.neg_inf, total_log_prob_rest)

        total_log_prob_rest = torch.logsumexp(total_log_prob_rest, dim=-1, keepdim=False)
...
class NegativeBinomialPoissonConv(TorchNegativeBinomialPoissonConv, dist.torch_distribution.TorchDistributionMixin):
    pass
