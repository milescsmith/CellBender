...
class TorchNegativeBinomialPoissonConvApprox(Distribution):
    ...
    def expand(self, batch_shape, _instance=None):
        new = self._get_checked_instance(TorchNegativeBinomialPoissonConvApprox, _instance)
        batch_shape = torch.Size(batch_shape)
        new.mu = self.mu.expand(batch_shape)
        new.alpha = self.alpha.expand(batch_shape)
        new.lam = self.lam.expand(batch_shape)

        super(TorchNegativeBinomialPoissonConvApprox, new).__init__(batch_shape, validate_args=False)
    ...
    def _poisson_log_prob_two(lam):
        return 2.0 * lam.log() - lam - 0.69314718  # ln(2)
    ...
    def log_prob(self, value):
        ...
        nb_approx_log_prob = self._neg_binom_log_prob(mu=mean_hat, alpha=alpha_hat, value=value)

        # Use a poisson for small mu, where the above approximation is bad.
        empty_indices = mu < 1e-5 * lam
        poisson_log_prob = self._poisson_log_prob(lam=lam[empty_indices], value=value[empty_indices])
...
class NegativeBinomialPoissonConvApprox(
    TorchNegativeBinomialPoissonConvApprox, dist.torch_distribution.TorchDistributionMixin
):
    pass
