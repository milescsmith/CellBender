...
class NullDist(TorchDistribution):
    ...
    def __init__(self, v, log_density=0.0, event_dim=0, validate_args=None):
        if event_dim > v.dim():
            msg = f"Expected event_dim <= v.dim(), actual {event_dim} vs {v.dim()}"
        ...
        if isinstance(log_density, numbers.Number):
            ...
            msg = f"Expected log_density.shape = {log_density.shape}, actual {batch_shape}"
            raise ValueError(msg)
        self.v = v
    ...
    def expand(self, batch_shape, _instance=None):
        ...
        super(NullDist, new).__init__(batch_shape, self.event_shape, validate_args=False)
