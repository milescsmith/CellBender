...
def load_or_compute_posterior_and_save(
    dataset_obj: "SingleCellRNACountsDataset", inferred_model: "RemoveBackgroundPyroModel", args: argparse.Namespace
) -> "Posterior":
    ...
    def _do_posterior_regularization(posterior: Posterior):
        ...
        if args.posterior_regularization is not None:
            if args.posterior_regularization == "PRq":
                posterior.regularize_posterior(regularization=PRq, alpha=args.prq_alpha, device="cuda")
...
class Posterior:
    ...
    def __init__(
        ...
        if dataset_obj is not None:
            self.index_converter = IndexConverter(
                total_n_cells=dataset_obj.data["matrix"].shape[0], total_n_genes=dataset_obj.data["matrix"].shape[1]
            )
    ...
    def compute_denoised_counts(self, estimator_constructor: EstimationMethod, **kwargs) -> sp.csc_matrix:
        ...
        noise_csr = estimator.estimate_noise(
            noise_log_prob_coo=posterior_coo, noise_offsets=self._noise_count_posterior_coo_offsets, **kwargs
        )
    ...
    def regularize_posterior(self, regularization: "PosteriorRegularization", **kwargs) -> sp.coo_matrix:
    ...
    def _get_cell_noise_count_posterior_coo(
        ...
        for i, data in enumerate(cell_data_loader):
            ...
            noise_log_pdf_NGC, noise_count_offset_NG = self.noise_log_pdf(
                data=data, n_samples=n_samples, y_map=y_map, n_counts_max=n_counts_max
            )
            ...
            bcs_i_chunk, genes_i_analyzed, c_i, log_prob_i = dense_to_sparse_op_torch(
                noise_log_pdf_NGC, tensor_for_nonzeros=tensor_for_nonzeros
            )
        ...
        self._noise_count_posterior_coo = sp.coo_matrix(
            (log_probs, (m, c)), shape=[np.prod(self.count_matrix_shape, dtype=np.uint64), n_counts_max]
        )
    ...
    def map_denoised_counts_from_sampled_latents(
        ...
        noise_log_pdf, offset_noise_counts = self.noise_log_pdf(
            data=data, n_samples=n_samples, lambda_multiplier=lambda_multiplier, y_map=y_map
        )
    ...
    def _log_prob_noise_count_tensor(
        ...
        if debug:
            ...
            if torch.isinf(log_prob_tensor).all(dim=-1).any():
                msg = "There is at least one log_prob_tensor[n, g, :] that has all-zero probability"
    ...
    def _get_latents_map(self):
        ...
        start = 0
        for _i, data in enumerate(data_loader):
    ...
    def _get_mu_alpha_lambda_map(self, data: torch.Tensor, chi_ambient: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        mu_map = calculate_mu(epsilon=epsilon, d_cell=d_cell, chi=chi_map, y=y, rho=rho)
...
class PosteriorRegularization(ABC):
    ...
    @staticmethod
    @abstractmethod
    def regularize(noise_count_posterior_coo: sp.coo_matrix, noise_offsets: dict[int, int], **kwargs) -> sp.coo_matrix:
...
class PRq(PosteriorRegularization):
    ...
    def _compute_log_target_dict(noise_count_posterior_coo: sp.coo_matrix, alpha: float) -> dict[int, float]:
        ...
        return dict(zip(result["m"], result["result"], strict=False))
    ...
    def _get_alpha_log_constraint_violation_given_beta(
        ...
        log_numerator_B = torch.logsumexp(
            noise_count_BC.log() + log_pdf_noise_counts_BC + beta_B.unsqueeze(-1) * noise_count_BC, dim=-1
        )
        log_denominator_B = torch.logsumexp(log_pdf_noise_counts_BC + beta_B.unsqueeze(-1) * noise_count_BC, dim=-1)
    ...
    def _chunked_compute_regularized_posterior(
        ...
        if n_chunks is None:
            dense_size_gb = (
                (
                    ...
                    * (noise_count_posterior_coo.shape[1] + np.array(list(noise_offsets.values())).max())
                )
                * 4
                / 1e9
            )  # GB
        ...
        for i in range(n_chunks):
            ...
            noise_count_BC = noise_count_BC + (
                torch.tensor([noise_offsets.get(m, 0) for m in m_indices_for_chunk], dtype=torch.float)
                .unsqueeze(-1)
                .to(device)
            )
            ...
            m_i, c_i, log_prob_reg_i = dense_to_sparse_op_torch(log_pdf_reg_BC, tensor_for_nonzeros=tensor_for_nonzeros)
    ...
    def regularize(
        ...
        **kwargs,
        ...
        log_target_dict = PRq._compute_log_target_dict(noise_count_posterior_coo=noise_count_posterior_coo, alpha=alpha)
    ...
    def _binary_search_for_posterior_regularization_factor(
        ...
        def summarize_map_noise_counts(x: torch.Tensor, per_gene: bool) -> torch.Tensor:
            ...
            map_noise_csr = estimator.estimate_noise(
                noise_log_prob_coo=regularized_noise_posterior_coo, noise_offsets=noise_offsets, device=device
            )
    ...
    def regularize(
        ...
        **kwargs,
        ...
        posterior_subset_coo = PRmu._subset_posterior_by_cells(
            noise_count_posterior_coo=noise_count_posterior_coo, index_converter=index_converter, n_cells=n_cells
        )
...
class IndexConverter:
    ...
    def get_m_indices(
        ...
        if type(cell_inds) == np.ndarray:
            ...
            msg = "IndexConverter.get_m_indices received cell_inds of unkown object type"
...
def compute_mean_target_removal_as_function(
    ...
    mean_noise_csr = estimator.estimate_noise(
        noise_log_prob_coo=noise_count_posterior_coo, noise_offsets=noise_offsets, device=device
    )
...
def restore_from_checkpoint(
    ...
    posterior = Posterior(dataset_obj=dataset_obj, vi_model=d["model"])
